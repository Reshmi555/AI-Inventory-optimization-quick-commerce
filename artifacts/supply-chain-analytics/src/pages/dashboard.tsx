...
function DashboardShell({ active, setActive, children, isDark, onToggleDark }: { active: SectionId; setActive: (section: SectionId) => void; children: React.ReactNode; isDark: boolean; onToggleDark: () => void }) {
  ...
  return <div className="min-h-[100dvh] bg-background">
    <aside className={`fixed inset-y-0 left-0 z-40 flex w-[252px] flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground transition-transform duration-200 lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex h-[78px] items-center justify-between border-b border-sidebar-border px-5">
        <button onClick={() => setActive("overview")} className="flex items-center gap-3 text-left" aria-label="Go to executive overview">
          <span className="grid size-9 place-items-center bg-sidebar-primary text-sidebar-primary-foreground"><Zap className="size-4" /></span>
           <span><span className="block text-[13px] font-extrabold tracking-[-0.02em]">Inventory<span className="text-sidebar-primary">/</span>OS</span><span className="font-mono text-[9px] uppercase tracking-[0.14em] text-sidebar-foreground/55">Decision intelligence</span></span>
        </button>
     ...
     {mobileOpen && <button className="fixed inset-0 z-30 bg-black/40 lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Close navigation overlay" />}
...
function Overview({ query }: { query: ReturnType<typeof useGetSupplyChainOverview> }) {
  ...
  return <div>
    ...
    <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
      ...
      <Card className="border-border/80 bg-card shadow-[0_8px_24px_rgba(25,48,70,0.04)]"><CardHeader className="border-b border-border/70 px-5 pb-3 pt-4"><CardTitle className="text-[15px]">Assessment context</CardTitle></CardHeader><CardContent className="grid grid-cols-2 gap-x-5 gap-y-5 px-5 py-5 sm:grid-cols-3"><Metric label="Model" value={data?.forecastModel || "—"} /><Metric label="Policy" value={data?.policyLabel || "—"} /><Metric label="Series" value={formatNumber(data?.totalSeries)} /><Metric label="Risk stores" value={formatNumber(data?.totalStores)} /><Metric label="Risk products" value={formatNumber(data?.totalProducts)} /><Metric label="Assessment date" value={formatDate(data?.assessmentDate)} /></CardContent></Card>
    </div>
  </div>;
}
...
function Forecast({ query, driversQuery, storeId, setStoreId, productId, setProductId, applyFilters }: { query: ReturnType<typeof useGetSupplyChainForecasts>; driversQuery: ReturnType<typeof useGetSupplyChainDrivers>; storeId: string; setStoreId: (value: string) => void; productId: string; setProductId: (value: string) => void; applyFilters: () => void }) {
  ...
  const chartData = useMemo(() => {
    const grouped = new Map<string, { date: string; observedDemand: number; forecastDemand: number; forecastLower: number; forecastUpper: number }>();
    for (const row of data) {
      const current = grouped.get(row.date) ?? { date: row.date, observedDemand: 0, forecastDemand: 0, forecastLower: 0, forecastUpper: 0 };
      current.observedDemand += row.observedDemand;
      current.forecastDemand += row.forecastDemand;
      current.forecastLower += row.forecastLower;
      current.forecastUpper += row.forecastUpper;
...
function AnalystResponse({ analyst, onRetry }: { analyst: ReturnType<typeof useAskSupplyChainAnalyst>; onRetry: () => void }) {
  ...
  if (analyst.isError) return <Card className="border-border/80 bg-card"><ErrorState onRetry={onRetry} /></Card>;
